...
#' convert_adjacency -  A function where you input some kind of adjacency object
...
#' or edgelist, it will not be correct: try running row_norm(A, reverse=TRUE).
...
convert_adjacency <- function(A, out="matrix", df=NULL){
  convert.to.igraph <- function(A2){
    if(class(A2)[1]=="matrix" & NCOL(A2) == NROW(A2)){
      ...
      step1 <- igraph::graph_from_adjacency_matrix(A2)
      outA2 <- step1

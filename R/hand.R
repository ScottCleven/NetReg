...
hand_likelihood <- function(theta, A, y, X, Lambda, Omega, log=TRUE){
  beta <- theta[NROW(theta)-2]
  ...
  mvtnorm::dmvnorm(y,
                   ...
                   as.matrix(crossprod(gamma)[1]*Omega + s2*tcrossprod(Launtif)),
                   log = log)

}

# Posterior of the effects model.
hand_posterior <- function(theta, A, y, X, Lambda, Omega, log=TRUE){
  beta <- theta[NROW(theta)-2]
  s2 <- theta[NROW(theta)-1]
  rho <- theta[NROW(theta)]
  ...
  out <- hand_likelihood(theta, A, y, X, Lambda, Omega, log=FALSE) *
...
hand_normal <- function(handdat){
  ...
  if(typeof(handdat$beta_prior) == "list"){
    if(length(handdat$beta_prior$mean) == 1){
      bmean <- rep(handdat$beta_prior$mean,p)
    }else{
      bmean <- handdat$beta_prior$mean
    }

    if(length(handdat$beta_prior$sigma) == 1){
      bsigma <- diag(p) * handdat$beta_prior$sigma
    }else{
      bsigma <- handdat$beta_prior$sigma
    }
  ...
  if(typeof(handdat$s2_prior) == "list"){
    s2prior <- function(s2){
      dgamma(handdat$s2_prior$shape, handdat$s2_prior$scale)
    }
  }else{
    s2prior <- handdat$s2_prior
  }

  if(typeof(handdat$rho_prior) == "list"){
  ...
  if(!is.null(handdat$gr)){
    gr=handdat$gr
  }else{
    gr=NULL
  }

  if(!is.null(handdat$lower)){
    lower = handdat$lower
  }else{
    lower=-Inf
  }

  if(!is.null(handdat$upper)){
    upper = handdat$upper
  }else{
    upper=Inf
  }

  if(!is.null(handdat$control)){
    control=handdat$control
  }else{
    control=list()
  }

  # Finding good initial beta values
  init.betas <- lm(handdat$y ~ handdat$X[,-1])$coefficients

  # Finding posterior estimates using the normal approximation of the posterior
  myopt <- optim(par=c(init.betas, 1, 0.36), fn=hand_posterior,
                 method = "BFGS",
                 lower=lower,
                 upper=upper,
                 control=control,
  ...
  list(beta = myopt$par[1:handdat$p], s2 = myopt$par[handdat$p + 1],
       rho = myopt$par[handdat$p + 2], network = handdat$network,
       H = myopt$hessian, Hinv = qr.solve(myopt$hessian), logpost = myopt$value,
       loglik = hand_likelihood(myopt$par, handdat$network, handdat$y, handdat$X),
       y = handdat$y, X = handdat$X, method="normal", model="effects")

}

# Code for when method="greta"
hand_greta <- function(handdat){
  ...
  if(typeof(handdat$rho_prior) == "list"){
    ...
    rho = handdat$rho_prior
  }


  # Checking if additional Arguments were input
  if(!is.null(handdat$n_samples)){
    n_samples=handdat$n_samples
  }else{
    n_samples=5000
  }
  ...
  if(!is.null(handdat$one_by_one)){
    one_by_one=handdat$one_by_one
  }else{
    one_by_one=TRUE
  }

  if(!is.null(handdat$initial_values)){
    initial_values=handdat$initial_values
  }else{
    initial_values=greta::initials(s2=1, rho=0.36)
  }

  # set likelihood
  Launtif <- solve(handdat$I - rho * handdat$network)
  mu <- handdat$X %*% beta
  Sigma <- s2*Launtif %*% t(Launtif)
  y <- t(matrix(handdat$y, ncol = 1))
  greta::distribution(y) <- greta::multivariate_normal(
    mean = t(mu),
    Sigma = Sigma,
    dimension = NROW(handdat$n))
